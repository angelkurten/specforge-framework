...
│   │   │   ├── hard-rules.md       ← the 16 invariants (unscoped)
...
│   │   ├── prd.md
│   │   ├── adr.md
│   │   ├── agdr.md                 ← optional Agent Decision Record (rare)
...
The fastest way to install specforge is the npm CLI — no clone required:
...
`init` scaffolds the complete framework layout (rules, templates, agents, examples), writes editable stubs for your team data (`SIBLINGS.md`, `ROADMAP.md`), and records `.specforge/manifest.json` — the installed version plus per-file hashes used for drift detection.

Lifecycle commands, all run from the specforge directory:
...
To attempt to bound where all fourteen `WebFetch`-holding definitions may fetch from, pair an `allow` entry for the domains you trust with a blanket `WebFetch` deny in the same `.claude/settings.json` — the `allow` half alone only pre-approves a domain and does not itself block any other:
...
4. **Run the review loop.** Dispatch the reviewer subagents in parallel by name — `specforge-backend-reviewer`, `specforge-frontend-reviewer`, `specforge-security-reviewer`, `specforge-quality-reviewer` — selecting the roles whose domain your PRD carries surface in, per the trigger table in [`workflow.md`](.claude/rules/workflow.md) step 5. Quality always runs; the other three fire on a trigger, so a single-domain PRD gets a two-reviewer panel rather than four. Each carries a structured brief with `REVIEW_MODE: draft`, the PRD path, the sibling's `CLAUDE.md` path, and the code references to verify against. Consolidate findings by severity. Re-dispatch *only* the domains with 🔴 blockers, with `REVIEW_MODE: re-verification` and a ledger of the findings that reviewer raised — never run a fresh review from scratch after fixes.
