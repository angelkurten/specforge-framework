...
description: Implements the frontend surface of a frozen specforge PRD — components, pages, state, client-side data flow, and their tests — against one sibling's codebase. Dispatched explicitly by the specforge workflow (step 9) with a structured brief — not intended for automatic delegation.
model: sonnet
