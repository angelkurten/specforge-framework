...
CODE_REFERENCES: <frontend code to verify against — static paths in draft mode, `git diff --name-only <commit_hash>` output in post-implementation mode, and a fix range's changed files on a step-9 amendment bounce, which is dispatched in `draft` mode>
