...
Launch reviewers **in parallel** via the `Agent` tool — a typical panel of 4 (backend, frontend, security, quality), adapted to the domain. Each reviewer is dispatched by name with `subagent_type: specforge-<role>-reviewer` (`specforge-backend-reviewer`, `specforge-frontend-reviewer`, `specforge-security-reviewer`, `specforge-quality-reviewer`). The definition body is the subagent's system prompt — nothing is read and pasted. The dispatch prompt carries the six brief fields, all required, as labelled lines:
...
- **Instructions to emit an AgDR** (`templates/agdr.md`) only for a high-blast-radius design decision the PRD did **not** specify — and to apply the deliberately high bar in `prd-authoring.md` § Optional artifact: Agent Decision Records. Most scopes emit none. An emitted AgDR is referenced by number in the gate-block comment; it does not gate promotion.
