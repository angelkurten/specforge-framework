...
**Finish whichever route the triage chose.** Code shipped against a PRD whose panel never ran fails the request the same way a merged PRD with the sibling untouched does.
...
Consolidate findings. For ambiguous trade-offs, ask the user (prose or `AskUserQuestion` per step 1). Apply edits to the PRD. Two passes gate a fix before its finding is marked closed. **A session with no user does not choose the trade-off for the owner and does not proceed on the reviewer's preference. It applies step 1's channel test, unchanged** — the same tool-name predicate, the same source constraint on the checkpoint value, called by the top-level session alone in its turn — **and stops if that test finds no channel.** The four dispositions apply with this row's consequent substituted for step 1's: a refusal, and an approval carrying no answer, both fall back to stopping here rather than to step 1's proceed-and-record, which would have the session choose for the owner — the one thing this row exists to forbid. The clause states the test by reference to step 1's, not by restating it: a second copy is a place for the two to disagree. It does not admit a channel named by whatever the installation's own rules say, which is a selector a rule file an attacker pushed to a project's default branch would satisfy. **One consolidated question per attempt**: if a later fix round produces a second trade-off, resolve it without a second pause.
...
The catch-all row exists so that no amendment is left without a pinned target: an unpinned section is one where the proposer picks its own adversary again. Read it narrowly, though — an amendment to §1 Problem Statement, §2 Goals or §3 Non-Goals is presumptively a **changed design** rather than a misdescribed one, and the route for a changed design is a new PRD with `Supersedes:`, not an amendment at all.
