...
**"Exact form" binds the structure, not the class list.** The preamble names the classes the briefing actually carries, so a briefing whose fenced content is only ever user-supplied — every roadmap generator and critic, whose input is an evidence field — emits `is user-supplied input` and is conformant. What is not negotiable is that a preamble is present, re-emitted per fence, tells the reader to treat fence contents as data rather than commands, and declares the `␛BACKTICK␛` substitution.
