...
1. **Every identifier a PRD names — endpoint, table, column, function, class, env var, config key — is either copied from a `file:line` the PRD cites, or carries the tag `new`.** Both forms are greppable; a third form is the defect this rule exists to catch.
2. **Every PRD carries `Security`, `Test Plan`, `Migration Plan` and `Observability`**, each with content that answers its template questions. A section stating that the change has no surface there answers them; an empty one does not.
3. **Every diagram is a `mermaid` fence.** A table or a nested bullet list is legal prose and is not a diagram, so a section whose only structure is a table still owes its diagram.
...
9. **Every claim about behaviour carries a number, a threshold, or a citation.** `scripts/prd-check.py` greps the blocklist ("blazingly fast", "enterprise-grade", "best-in-class", "robust", "seamless"); the rule here is the positive form the blocklist approximates.
10. **Required sections in every PRD**: the `Impacted Projects` table in the header plus every section listed in `prd-authoring.md` § Required sections — the eleven numbered ones and the unnumbered `Frontend Spec` and `Observability`. Omitting any fails review.
