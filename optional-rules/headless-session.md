...
| **Step 9 — escalation** | Option (ii): leave the PRD at `Draft` and ungated — the gate block keeps its `[TBD]` placeholders — explain why at the top, and stop. The same default covers the two other step-9 dead ends a headless session can reach: **validation that cannot be run** (a `VALIDATION: not run: <reason>` line blocks promotion, and the waiver needs a human), and **an amendment the bounce refuted** (stop without re-proposing; option (i)'s single re-proposal is a human's call, not the session's). Never option (iii) — waiving a finding is a human act, and a session with no human cannot perform it on one's behalf. |
