...
**Those six do not isolate the scoping sentences as the sole guard, and no cell can.** Every one of the six named a second, independent reason alongside the scoping: the catalog's checkpoint did not match the escalation either. The two conditions are co-extensive by construction — a checkpoint value matching the post-implementation escalation is only *at* that escalation when the session is, so a host setup that satisfies one guard and defeats the other does not exist to be built. What the six establish is that the scoping holds under maximum temptation and that sessions cite it as a reason; what they cannot establish is what the cell would do with the scoping sentences removed.
